(function($, undefined) {
	$.fn.fcbkmcomplete = function(opt) {
		return this.queue(function() {
			function init() {createFCBK();
				addInput(0)
			}

			function createFCBK() {
				holder1 = $('<ul class="holder1"></ul>').width(options.width);
				if(options.attachto) {
					if( typeof (options.attachto) == "object") {
						options.attachto.append(holder1)
					} else {
						$(options.attachto).append(holder1)
					}
				} else {
					element.after(holder1)
				}
				// complete = $('<br/><br/><div class="facebook-auto">').width(options.width);
				complete = $('<div class="facebook-auto" style = "width: auto; top : 27px; ">');
				complete.append('<div class="default" >Loding ......</div>').hide();
				...
				holder1.after(complete.prepend(feed));
				elPrepare()
			}
			...
			function elPrepare() {
				...
				$(element).bind("removeItem", function(event, data) {
					var item = holder1.children('li[rel=' + data.value + ']');
					if(item.length) {removeItem(item)
					}
				});
				...
				$(element).bind("selectAll", function(event, data) {
					...
					$.each($(element).data('cache'), function(key, value) {
						if($.inArray(key, currVals) === -1) {addItem(value, key, 0, 0, 0)
						}
					});
					feed.parent().hide()
				})
			...
			function addItem(title, value, preadded, locked, focusme) {
				...
				holder1.append(li);
				// holder1.append(li);
				aclose.click(function() {removeItem($(this).parent("li"));
					return false
				});
				if(!preadded) {
				...
				holder1.children("li.bit-box.deleted").removeClass("deleted");
				clear_feed(1);
				return id
			}
			...
			function addInput(focusme) {
				...
				holder1.append(li.append(input));
				input.focus(function() {
					isactive = true;
					if(maxItems()) {
						complete.fadeIn("fast")
						complete.children(".default").hide(); //change
					}
				});
				input.blur(function() {
					isactive = false;
				...
				holder1.click(function() {
					// if(options.input_min_size < 0 && feed.length) {load_feed(xssPrevent(input.val(), 1))
					// }
					// input.focus();
				...
				input.keyup(function(event) {
					...
					if(event.keyCode == _key.backspace && etext.length == 0) {clear_feed(1);
						complete.children(".default").hide();
						if(!holder1.children("li.bit-box:last").hasClass('locked')) {
							if(holder1.children("li.bit-box.deleted").length == 0) {
								holder1.children("li.bit-box:last").addClass("deleted");
								return false
							} else {
								if(deleting) {
									return
								...
								deleting = 1;
								holder1.children("li.bit-box.deleted").fadeOut("fast", function() {removeItem($(this));
									return false
								})
							}
						}
					}
					if(event.keyCode != _key.downarrow && event.keyCode != _key.uparrow && event.keyCode != _key.leftarrow && event.keyCode != _key.rightarrow && etext.length > options.input_min_size) {load_feed(etext);
						complete.children(".default").show();
						feed.show()
					}
				...
				if(options.oncreate) {funCall(options.oncreate, input)
				}
				if(focusme) {setTimeout(function() {
						input.focus();
						complete.children(".default").show()
					}, 1)
				}
			...
			function removeFeedEvent() {
				feed.unbind("mouseover").unbind("mouseout").mousemove(function() {bindFeedEvent();
			...
			function bindEvents() {
				...
				maininput.keydown(function(event) {
					...
					if(event.keyCode == _key.downarrow) {nextItem('first')
					}
					if(event.keyCode == _key.uparrow) {nextItem('last')
					}
				})
			}

			function nextItem(position) {removeFeedEvent();
			...
			function maxItems() {
				return options.maxitems != 0 && (holder1.children("li.bit-box").length < options.maxitems)
			}
			...
			function load_feed(etext) {
				...
				if(options.json_url && maxItems()) {
					if(options.cache && json_cache_object.get(etext)) {addMembers(etext);
						bindEvents()
					} else {
						getBoxTimeout++;
