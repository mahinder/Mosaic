...
#    prawnto :prawn => {:left_margin => 25, :right_margin => 25}
    before_filter :protect_other_employee_data, :only => [:emp_attendance]
    ...
    def add_leave_types
        ...
        @record_count = EmployeeLeaveType.count(:all)
       if request.post? and @leave_type.save
            ...
            response = { :leave_types => @leave_types, :inactive_leave_types => @inactive_leave_types, :record_count => @record_count,:valid => true, :notice => 'Employee Leave was successfully created.'}
    ...
    def ajax_add_leaves_types
       ...
       if @leave_type.save
        ...
        response = {:leave_types => @leave_types, :inactive_leave_types => @inactive_leave_types, :record_count => @record_count,:valid => false,:errors=> @leave_type.errors}
      end
    respond_to do |format|
      format.html # all.html.erb
      format.json  { render :json => response }
   ...
   @leave_types = EmployeeLeaveType.find(:all, :order => "name ASC",:conditions=>'status = 1')
        @inactive_leave_types = EmployeeLeaveType.find(:all, :order => "name ASC",:conditions=>'status = 0') 
   @record_count = EmployeeLeaveType.count(:all)
 response = { :leave_types => @leave_types, :inactive_leave_types => @inactive_leave_types, :record_count => @record_count}
  respond_to do |format|
    ...
    def edit_leave_types
        ...
        respond_to do |format|
          if @leave_type.update_attributes(params[:leave_type])
            format.html { redirect_to @leave_type, notice: 'Employee Leave was successfully updated.' }
        format.json { render :json => {:valid => true, :leave_type => @leave_type, :notice => "Employee Leave was updated successfully."} }
      else
        format.html { render action: "edit" }
        format.json { render :json => {:valid => false, :leave_type.errors => @leave_type.errors}}
    ...
    def delete_leave_types
        ...
        if @attendance.blank?
            ...
            @leave_count.each do |e|
             ...
             format.json { render :json => {:valid => true, :leave_type => @leave_type, :notice => "Leave type deleted successfully"} }
            
        else
          str = "Sorry! You Cannot delete a leave type with leave entry"
      dependency_errors = {:dependency => [*str]}
          format.json { render :json => {:valid => false, :leave_type => @leave_type,:errors=>dependency_errors} }
    ...
    def leave_reset_settings
        ...
        @auto_reset = SchoolConfiguration.find_by_config_key('AutomaticLeaveReset')
        @reset_period = SchoolConfiguration.find_by_config_key('LeaveResetPeriod')
        @last_reset = SchoolConfiguration.find_by_config_key('LastAutoLeaveReset')
        @fin_start_date = SchoolConfiguration.find_by_config_key('FinancialYearStartDate')
        if request.post?
    ...
    def update_employee_leave_reset_all
        ...
        @leave_count.each do |e|
            ...
            puts "the employee leave type is#{e.employee_leave_type_id}"
        ...
        render :text => "The Leave count is reset for all employees successfully"
    ...
    def employees_list
        ...
        render :partial => 'employee_view_all_list', :object => @employees
    ...
    def employee_wise_leave_reset
        ...
        render :partial => 'employee_reset_sucess'
    ...
    def update_leave_history
        ...
        render :partial => 'update_leave_history'
        # render :update do |page|
    ...
    def employee_attendance_pdf
        ...
        render :pdf => 'employee_attendance_pdf',:layout => "layouts/pdf.html.erb",:header => {:html =>{:template=> 'layouts/pdf_header.html.erb'}},:footer => {:html => { :template=> 'layouts/pdf_footer.html.erb'}},:margin => {:top=> 40,
                    :bottom => 20,
                    :left=> 30,
