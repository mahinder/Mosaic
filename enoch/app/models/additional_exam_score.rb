...
class AdditionalExamScore < ActiveRecord::Base
  ...
  def calculate_grade
    ...
    unless additional_exam_type == 'grades'
      unless self.marks.nil?
        ...
        if additional_exam_type == 'MarksAndGrades'
       unless additional_exam_group.exam_group.nil?
        grade = GradingLevelDetail.percentage_to_grade(percent_score, additional_exam_group.exam_group.grading_level_group.id)
        else
           grade = GradingLevelDetail.percentage_to_grade(percent_score, 1)
        end
        end
        self.grading_level_detail_id = grade.id if additional_exam_type == 'MarksAndGrades'
      else
  ...
  def calculate_pass
   additional_exam = self.additional_exam
    additional_exam_group = additional_exam.additional_exam_group
    additional_exam_type = additional_exam_group.exam_type
    unless additional_exam_type=='Grades' || additional_exam_type=='Custom'
