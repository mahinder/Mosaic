...
class FinanceFeeCollection < ActiveRecord::Base
  ...
  validate :validate
  # validates :name,:start_date,:fee_category_id,:end_date,:due_date
  ...
  def validate
    unless self.start_date.nil? or self.end_date.nil?      
      errors.add(:start_date ,"cant be after end date") if self.start_date > self.end_date
      errors.add(:start_date ,"cant be after due date") if self.start_date > self.due_date
      errors.add(:end_date ,"cant be after due date") if self.end_date > self.due_date
    else   
    end
  end
  ...
  def create_associates
    ...
    batch_discounts = BatchFeeDiscount.find_all_by_finance_fee_category_id_and_batch_id(self.fee_category_id,self.batch_id)
    ...
    category_discount = StudentCategoryFeeDiscount.find_all_by_finance_fee_category_id_and_batch_id(self.fee_category_id,self.batch_id)
  ...
  student_discount = StudentFeeDiscount.find_all_by_finance_fee_category_id_and_batch_id(self.fee_category_id,self.batch_id)
  ...
  particlulars = FinanceFeeParticulars.find_all_by_finance_fee_category_id(self.fee_category_id,:conditions=> {:is_deleted => 'f'})
  ...
  def fees_particulars(student)
    FeeCollectionParticular.find_all_by_finance_fee_collection_id(self.id,
      :conditions => ["((student_category_id IS NULL AND admission_no IS NULL )OR(student_category_id = '#{student.student_category_id}'AND admission_no IS NULL) OR (student_category_id IS NULL AND find_in_set('#{student.admission_no}',admission_no) <> 0)) and is_deleted=0"])
  end
